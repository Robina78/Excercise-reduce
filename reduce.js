...
function extractValue(arr, key) {
 return arr.reduce(function(extract,nextEle){
        extract.push(nextEle[key]);
        return extract;
    },[]);  
}
...
function vowelCount(str) {
    const vowels = 'aeiou';
    return str.split('').reduce(function(frequent,nextChar){
      let lowerCased = nextChar.toLowerCase();
      if (vowels.indexOf(lowerCased) !== -1){
          if(frequent[lowerCased]) {
            frequent[lowerCased]++ ;
          } else {
            frequent[lowerCased] = 1
          }          
      }
      return frequent;
...
function addKeyAndValue(arr, key, value) {
  return arr.reduce(function(addEle,nextEle,idx){
        addEle[idx][key] = value;
        return addEle;
   },arr)
